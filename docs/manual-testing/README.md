...
| S13  | 内置文案配置与本地化          | 每表局部 `texts`、默认回退、动态 formatter、Final 与可访问名称。                       |
...
不计入下方已经执行的 23 个本地正式用例，也不能通过本地运行触发真实部署。

## 已执行正式用例

结果定义：通过 = 实际与预期一致；阻塞 = 无法执行且非产品失败。本轮共 23 个正式用例：23 通过、0 失败、0 阻塞；其中 MT-18 通过但产生文档 Finding。
...
| MT-23 | S13；打开 `#example-text-overrides`，以键盘操作并用读屏核对可访问名称；再检查 Advanced。              | 局部 partial `texts` 保留 default fallback；long property 与 version formatter 上下文完整；Final source、presence、edit、validation 文案依序可辨。 | `merge.finalLabel > texts.finalColumn > Final`；空字符串保留、undefined 回退。`ConfigProvider` 仍负责 Ant Design locale；用户 label/文案不被接管、翻译或 override。通过。  | Demo、组件文本测试与 ARIA 查询                             |
