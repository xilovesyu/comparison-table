...
结果定义：通过 = 实际与预期一致；阻塞 = 无法执行且非产品失败。本轮共 19 个正式用例：18 通过、0 失败、1 阻塞（MT-19）；其中 MT-18 通过但产生文档 Finding。
...
| MT-19 | 基础可用性；按 S9（空对象、1000+ 行、10k 字符值）运行首渲染、搜索、横向滚动与空状态路径。 | 页面提供可复用的非生产数据入口，能记录时间和 UI 状态。                                                 | 阻塞：当前 Demo 只提供业务样本，未提供空/长文本/大数据的测试入口；为避免新添文档示例而连带修改综合案例、源码及测试，本轮未临时改产品 Demo。 | 未覆盖风险                                              |
...
- 体验问题（P3）：页面一次性渲染 11 张完整表格。首次进入对新用户来说信息量很大，且每卡都有相同工具栏；建议将示例目录/锚点导航或折叠卡片作为文档体验改进，保持当前 API 不变。
...
新增 Demo 示例时，遵守仓库 `AGENTS.md`：示例列在综合高级配置之前、能力接入综合案例、源码面板和测试同步更新。新增能力必须补一条对应的 MT 用例和一个可复现证据链接。
