...
| S12  | Final 最终版本合并            | 默认关闭；controlled/uncontrolled 决策、keyed presence Include/Exclude 与 raw 结果。   |
...
不计入下方已经执行的 21 个本地正式用例，也不能通过本地运行触发真实部署。

## 已执行正式用例

结果定义：通过 = 实际与预期一致；阻塞 = 无法执行且非产品失败。本轮共 21 个正式用例：21 通过、0 失败、0 阻塞；其中 MT-18 通过但产生文档 Finding。
