...
converted to timing values. R-7 summaries use linear interpolation and report finite min, median, p95, and
max values with no pass/fail threshold.
...
The private runner exposes only the `stages` injection point for catalog, environment, measurement,
summarize, and report stages so
