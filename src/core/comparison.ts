...
function row(key: string, path: (string | number)[], values: unknown[], versions: readonly ComparisonVersion[], context: PropertyContext, children?: ComparisonRow[], displayRule?: DisplayRule, def?: PropertyDefinition): ComparisonRow { const property: PropertyDefinition = { key, label: displayRule?.label ?? def?.label ?? key, path, level: path.length - 1, type: context.type, renderer: displayRule?.renderer ?? def?.renderer, ...def }; return { id: pathId(path), property, values: Object.fromEntries(versions.map((v, i) => [v.id, values[i]])), children: children?.length ? children : undefined }; }
